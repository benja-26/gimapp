 ...
 * Modal, Field, Input, Select, BtnRow, Badge, KCard, KRow, TxItem, Tabs, SecH
 */
...
export function Select({ children, ...props }) {
  return <select style={inputSt} {...props}>{children}</select>
}
  ...
  return (
    <div style={{ display: "flex", gap: 9, marginTop: 18 }}>
      <button onClick={onCancel} style={{
        flex: 1, background: "none", border: "1px solid #252d3d", borderRadius: 9,
        color: "#8891a8", padding: 13, cursor: "pointer", fontSize: 14,
        fontFamily: "'DM Sans',sans-serif",
      }}>
        Cancelar
      </button>
      <button onClick={onSave} style={{
        flex: 2, background: "#ff5722", border: "none", borderRadius: 9,
        color: "#fff", padding: 13, cursor: "pointer", fontSize: 15,
...
export function KCard({ label, value, sub, color = "#ff5722" }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, color: "#8891a8", textTransform: "uppercase", letterSpacing: .4, marginBottom: 7 }}>{label}</div>
...
export function KRow({ label, value, color = "#eef0f8", sub }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 700, color }}>{value}</div>
    </div>
  )
}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {onDelete && (
          <button onClick={onDelete} style={{
            background: "none", border: "none", color: "#3e4658",
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
      ...
      {action && (
        <button onClick={onAction} style={{
          fontSize: 12, color: "#ff5722", background: "none",
...
export const SubTitle = ({ children }) => (
  <div style={{ fontSize: 12, color: "#8891a8", marginBottom: 14 }}>{children}</div>
)

export const Empty = ({ icon = "📭", text = "Sin registros" }) => (
